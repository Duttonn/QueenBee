# 🐝 QUEEN BEE: PROTOCOLE DE SOUDURE (PRD)

> **Objectif Unique** : Transformer l'UI "Mock" en une interface de contrôle temps réel pilotée par le Backend.
...
### A. Flux de Données (Event-Driven)
...
### B. Isolation des Agents (Git Worktrees)
...
## 2. DÉCOUPAGE DES TÂCHES (GRANULARITÉ FINE)

### 🔵 DOMAINE : FRONTEND (REACT/ELECTRON)

**F-01 : Nettoyage des Mocks**
- **Fichier** : `src/store/useHiveStore.ts`
- **Action** : Supprimer `INITIAL_THREADS` et `MOCK_PROJECTS`. Initialiser les états à `[]` ou `null`.
- **Validation** : L'app démarre vide (c'est normal).

**F-02 : Service Natif**
- **Fichier** : `src/services/native.ts` (Nouveau)
- **Action** : Créer une classe qui vérifie `if (window.electron)` avant d'appeler l'IPC. Sinon, retourner des erreurs explicites "Web Mode - Feature Unavailable".

**F-03 : Écouteur Universel**
- **Fichier** : `src/hooks/useSocketEvents.ts`
- **Action** : Écouter :
  - `FILE_CHANGE` -> `refreshDiff()`
  - `QUEEN_STATUS` -> `setAgentStatus()`
  - `TERMINAL_DATA` -> `xterm.write()`

### 🟠 DOMAINE : BACKEND (NODE/PYTHON)

**B-01 : IPC Exposition**
- **Fichier** : `electron/preload.ts`
- **Action** : Exposer via `contextBridge` :
  - `fs`: `readFile`, `writeFile`, `readDir`
  - `shell`: `openExternal`, `showItemInFolder`
  - `git`: `status`, `diff` (appels wrapper)

**B-02 : Tool Executor (Le Cerveau)**
- **Fichier** : `src/lib/ToolExecutor.ts` (Nouveau)
- **Action** : Implémenter un interpréteur de commandes JSON venant du LLM :
  ```json
