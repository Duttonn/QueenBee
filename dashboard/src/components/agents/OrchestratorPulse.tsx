...
const OrchestratorPulse = () => {
  ...
  return (
    <div className="flex flex-col gap-4 p-6 bg-slate-900 rounded-xl border border-slate-700 shadow-xl">
      <div className="flex justify-between items-center border-b border-slate-800 pb-4">
        <h3 className="text-lg font-mono text-blue-400">⚡ Execution Pipeline</h3>
        <span className="text-xs bg-blue-500/20 text-blue-400 px-2 py-1 rounded">PROMPT_DRAFTING</span>
      </div>
        ...
        {activeSteps.map((step, idx) => (
          <div key={step.id} className="flex items-start gap-4 relative z-10">
            ...
            <div className="flex flex-col">
              ...
              {step.status === 'active' && (
                <div className="mt-2 bg-slate-800 p-3 rounded-md text-xs text-slate-400 border-l-2 border-blue-500">
                  <span className="text-blue-400 font-bold">DRAFTING:</span> "You are a visionOS expert. Refactor projection.js to use SIMD math for improved performance..."
            ...
            {idx < activeSteps.length - 1 && (
              <div className="absolute left-[5px] top-4 w-[2px] h-8 bg-slate-800" />
            )}
          </div>
        ))}
      </div>
