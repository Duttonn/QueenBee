...
import { UnifiedLLMService, unifiedLLMService } from '../UnifiedLLMService';
import { logContext } from '../infrastructure/logger';
...
import { CompletionGate } from '../infrastructure/CompletionGate';
...
export class AutonomousRunner {
  ...
  private lifecycleHooks: LifecycleHookManager;
    ...
    constructor(
      ...
      this.contextCompressor = new ContextCompressor(6, projectPath);
...
If you finish this task, stop and wait for the next command.`;
        ...
        // DeepAgents: Review & Adapt (every 3 tool executions)
        const toolExecutionCount = this.session!.messages.filter(m => m.role === 'tool').length;
        if (toolExecutionCount > 0 && toolExecutionCount % 3 === 0) {
            console.log('[DeepAgents] Reviewing and Adapting plan...');
            result = await this.session!.prompt("Review your progress and adapt the plan if necessary. Ensure the task is still on track.", options);
        }
    ...
    if (this.role !== 'architect') {
      ...
      while (gateAttempt < MAX_GATE_RETRIES && !gatePassed) {
        try {
          const gate = new CompletionGate(this.projectPath, this.llmService, { timeoutMs: 20_000 });
