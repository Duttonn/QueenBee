 ...
 *   6. LLM-as-Judge — evaluates solution quality (P20-01)
 *   7. Coverage Threshold Check — blocks if code coverage falls below threshold (metaswarm pattern)
 *
...
export class CompletionGate {
  ...
  constructor(projectPath: string, llmService: UnifiedLLMService, options: { timeoutMs?: number } = {}) {
  ...
  async check(agentClaims?: AgentClaim, judgeInput?: JudgeInput): Promise<GateResult> {
    ...
    if (judgeInput) {
      const judgeCp = await this.runCheckpoint('llm_judge', async () => {
        const judge = new LLMJudge(this.llmService);
        judgeResult = await judge.judge(judgeInput);
    ...
    // Checkpoint 7: Coverage threshold check (metaswarm pattern)
    const coverageCp = await this.runCheckpoint('coverage', () => this.checkCoverage(blockers));
    checkpoints.push(coverageCp);
  ...
  /**
   * Checkpoint 7: Coverage threshold check.
   * Reads `.coverage-thresholds.json` from the project root and blocks if
   * the actual coverage (from coverage-summary.json) falls below any threshold.
   *
   * .coverage-thresholds.json format:
   *   { "lines": 80, "functions": 75, "branches": 70, "statements": 80 }
   *
   * metaswarm pattern: prevents shipping code below agreed coverage targets.
   */
  async checkCoverage(blockers: string[]): Promise<void> {
    const thresholdsPath = path.join(this.projectPath, '.coverage-thresholds.json');
    if (!(await fs.pathExists(thresholdsPath))) return; // No threshold file → skip

    let thresholds: Record<string, number>;
    try {
      thresholds = await fs.readJson(thresholdsPath);
    } catch {
      return; // Malformed threshold file → skip
    }

    // Look for coverage-summary.json produced by Jest / Vitest --coverage
    const summaryPaths = [
      path.join(this.projectPath, 'coverage', 'coverage-summary.json'),
      path.join(this.projectPath, 'coverage-summary.json'),
    ];

    let summaryPath: string | null = null;
    for (const p of summaryPaths) {
      if (await fs.pathExists(p)) { summaryPath = p; break; }
    }

    if (!summaryPath) {
      // No coverage report yet — run tests with coverage if test:coverage script exists
      const pkgPath = path.join(this.projectPath, 'package.json');
      if (await fs.pathExists(pkgPath)) {
        const pkg = await fs.readJson(pkgPath);
        const coverageScript = pkg.scripts?.['test:coverage'] || pkg.scripts?.['coverage'];
        if (coverageScript) {
          try {
            await execAsync('npm run test:coverage --if-present 2>&1', {
              cwd: this.projectPath,
              timeout: this.timeoutMs,
              env: { ...process.env, CI: 'true' },
            });
            ...
            // Coverage generation failed — skip check
            return;
          }
        }
      }
      if (!summaryPath) return; // Still no summary → skip
    }
    ...
    try {
      ...
      for (const [metric, threshold] of Object.entries(thresholds)) {
        ...
        if (actual < (threshold as number)) {
          failures.push(`${metric}: ${actual.toFixed(1)}% < ${threshold}% required`);
        }
      }

      if (failures.length > 0) {
        blockers.push(`Coverage below threshold: ${failures.join(', ')}. Add tests before marking DONE.`);
      }
    } catch {
      // Malformed summary — skip check
    }
  }
}
